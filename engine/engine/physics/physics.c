...
{    
    ...
    for (int si = 0; si < physics_system->num_archetypes; ++si)
    {
        ...
        int physics_data_i = Archetype_find_component_list(archetype, COMPONENT_PhysicsData);
        PhysicsData* physics_datas = archetype->component_lists[physics_data_i];
        ...
        for (int i = 0; i < archetype->entity_count; ++i)
        {
            ...
            // TODO: Function for zeroing?
            physics_data->force = (V3){ 0.f, 0.f, 0.f };

            // TODO: In my old engine, the velocity is updated as a separate step, then the position is computed.
