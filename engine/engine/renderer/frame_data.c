...
Status frame_data_init(FrameData* frame_data, Scene* scene)
{
	...
	// Transform Stage
	resize_float_buffer(&frame_data->view_space_bounding_spheres, STRIDE_SPHERE * scene->mesh_instances.count);
	...
	resize_float_buffer(&frame_data->vertex_lighting, total_faces * STRIDE_FACE_VERTICES * STRIDE_COLOUR);
